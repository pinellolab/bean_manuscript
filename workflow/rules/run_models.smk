...
rule run_bean_tiling:
    ...
    output:
        res="results/model_runs/bean/bean_run_result.bean_count_LDLRCDS_annotated{cutoff_suffix}/bean_element_result.MultiMixtureNormal.csv",
    run:
...
rule run_bean_tiling_acc:
    input:
        ...
        splice_site="resources/LDLR/LDLR_ABE_splice_targets.csv"
    output:
        res="results/model_runs/bean/bean_run_result.bean_count_LDLRCDS_annotated{cutoff_suffix}/bean_element_result.MultiMixtureNormal+Acc.csv",
    run:
...
rule run_bean_tiling_negctrl:
    ...
    run:
        shell("bean-run tiling {input.input_h5ad} -o results/model_runs/bean_negctrl --allele-df-key sig_allele_counts_spacer_2_7_A.G_translated_prop0.05_0.1 --splice-site-path {input.splice_site} --control-guide-tag ABE_CONTROL --cuda --fit-negctrl --negctrl-col Region --negctrl-col-value 'ABE control'")
...
rule evaluate_tiling_negctrl:
    input:
        ...
        mixnormal_res="results/model_runs/bean_negctrl/bean_run_result.bean_count_{tiling_lib}_annotated{cutoff_suffix}/bean_element_result.MultiMixtureNormal.csv",
        mixnormal_acc_res="results/model_runs/bean_negctrl/bean_run_result.bean_count_{tiling_lib}_annotated{cutoff_suffix}/bean_element_result.MultiMixtureNormal+Acc.csv",
    ...
    run:
        shell("python scripts/evaluate_model_runs/get_performance_tiling.py bean_count_{wildcards.tiling_lib}_annotated{wildcards.cutoff_suffix} --result-suffix _negctrl --noallele-screen-name bean_count_{wildcards.tiling_lib}_annotated")
