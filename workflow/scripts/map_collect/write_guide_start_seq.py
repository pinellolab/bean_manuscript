...
def rep_to_guide_start_seq(lib, rep):
    return (
        "GGAAAGGACGAAACACCG"
        if lib in ["LDLvar", "LDLRCDS"] and rep in [1, 2, 3, 4]
        else ""
    )
...
def write_guide_start_seqs_file(path, lib):
    with open(path, "w") as f:
        for r in ALL_REPS[lib]:
            for b in reps_to_bins(lib, r):
                f.write(f"rep{r}_{b},{rep_to_guide_start_seq(lib, r)}\n")
