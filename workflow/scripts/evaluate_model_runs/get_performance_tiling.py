...
model_ids = ["MultiMixtureNormal", "MultiMixtureNormal+Acc", "Normal_allEdited", "Normal_behive", ]
...
def get_bean_results(result_path_format):
    ...
    for model_id in model_ids:
        ...
        if 'edit' not in tbl.columns:
            ...
            if cols[1].startswith("target_"): cols[1] = "edit"
            elif cols[0].startswith("target_"): cols[0] = "edit"
            else:
                raise ValueError(f"Don't know the target element column in {result_path_format.format(model_id)}.")
            tbl.columns = cols
        ...
        if res_tbl is None:
            ...
            res_tbl = res_tbl.merge(tbl, left_on='edit', right_on=f'edit_{model_id}', how='outer')
            print(res_tbl)
        
    res_tbl = res_tbl.rename(
        columns={f"edit": "variant", f"group_{model_ids[0]}": "group", }
    )
    return res_tbl
...
def get_mageck_results(mageck_prefix):
    ...
    def read_and_add_suffix(disp_mode):
        ...
        if gene_order is None: gene_order = tbl.Gene.tolist()
...
def get_performances(
    ...
    def get_cols(bean_pattern, mageck_pattern, mageck_rra_pattern, mageck_suffixes = mageck_suffixes):
        bean_cols = [bean_pattern.format(m) for m in model_ids]
        mageck_cols=[
                mageck_pattern.format(disp_mode)
    ...
    kwargs = {
        "res_tbl": cmp_results,
        "z_cols": get_cols("mu_z_{}", "sort_num|z_{}", "pos|lfc_rra_{}",),
        "fdr_cols": get_cols(
            "fdr_dec_{}", "sort_num|fdr_{}", "pos|fdr_rra_{}"
        ),
...
def main():
    ...
    for annot_type in ["allEdited", "behive"]:
        mageck_prefix = (
            f"results/model_runs/mageck{args.result_suffix}/{args.screen_name}.target_{annot_type}/"
        )
    ...
    all_results.to_csv(f"results/model_runs/{args.screen_name}/all_scores{args.result_suffix}.csv")
    ...
    perf_df_p.to_csv(f"results/model_runs/{args.screen_name}/clinvar_pb{args.result_suffix}.metrics.csv")
    perf_df_plp.to_csv(
